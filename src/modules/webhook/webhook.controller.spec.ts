...
describe('WebhookController', () => {
  ...
  beforeEach(async () => {
      ...
      providers: [
        {
          ...
          useValue: {
            createWebhookRequest: jest
              .fn()
              .mockImplementation(() => Promise.resolve()),
            processCoordinates: jest
              .fn()
              .mockImplementation(() => Promise.resolve()),
        ...
        },
        {
          ...
          useValue: {
            processLandmarksByCoordinates: jest
              .fn()
              .mockImplementation(() => Promise.resolve()),
          },
        },
    ...
    landmarksProcessorService = module.get<LandmarksProcessorService>(
      LandmarksProcessorService,
    )
  ...
  describe('webhookRequestSync', () => {
    it('should process coordinates synchronously and return success response', async () => {
      ...
      expect(
        landmarksProcessorService.processLandmarksByCoordinates,
      ).toHaveBeenCalledWith(dto.lat, dto.lng, dto.radius)
      expect(webhookService.markAsCompleted).toHaveBeenCalledWith(
        'test-uuid-789',
      )
      expect(result).toEqual({
        success: true,
    ...
    it('should use default radius for sync request when not provided', async () => {
      ...
      expect(
        landmarksProcessorService.processLandmarksByCoordinates,
      ).toHaveBeenCalledWith(dto.lat, dto.lng, DEFAULT_SEARCH_RADIUS)
    })

    it('should handle errors in sync processing', async () => {
      const error = new Error('Overpass API error')
      jest
        .spyOn(landmarksProcessorService, 'processLandmarksByCoordinates')
  ...
  describe('getWebhookStatus', () => {
    it('should return webhook status when found', async () => {
      ...
      webhookService.getWebhookStatus = jest
        .fn()
